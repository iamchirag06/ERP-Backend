...
public class AdminController {
    ...
    public ResponseEntity<?> updateStudent(@PathVariable UUID id, @RequestBody Map<String, Object> updates) {
        ...
        if (updates.containsKey("cgpa")) {
            // Handle Double conversion safely from JSON
            Object cgpa = updates.get("cgpa");
            if (cgpa instanceof Number) {
                student.setCgpa(((Number) cgpa).doubleValue());
            }
        }
        if (updates.containsKey("guardianPhone")) student.setGuardianPhone((String) updates.get("guardianPhone"));
        if (updates.containsKey("activeBacklogs")) student.setActiveBacklogs((Integer) updates.get("activeBacklogs"));
